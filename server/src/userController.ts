...
const getGameHistoryByUUID = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
...
const register = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const accessToken = jwt.sign(
      { uuid: newUser.uuid, username: newUser.username },
      ACCESS_TOKEN_SECRET,
      { expiresIn: "1h" }
    );
...
const getUsersByUUIDs = async (req: Request, res: Response): Promise<void> => {
  try {
    let uuids: string[] = req.body.uuids;
