...
const authenticateToken = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
  ...
  jwt.verify(token, ACCESS_TOKEN_SECRET, async (err, decoded) => {
    ...
    req.user = decoded as jwt.JwtPayload; // Attach user data to request
    next();
  });
};
...
const refreshToken = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
  try {
    const refreshToken = req.headers["x-refresh-token"] as string; // Get refresh token from headers
    if (!refreshToken) {
      return res.status(401).json({ message: "Refresh token is required." });
    }

    const user = await User.findOne({ where: { refreshToken } as any }); // Cast to any to avoid type issues
    if (!user) {
    ...
    jwt.verify(refreshToken, REFRESH_TOKEN_SECRET, (err: jwt.VerifyErrors | null, decoded: any) => {
      ...
      req.user = decoded as jwt.JwtPayload; // Attach user data
      next();
    });
  } catch (err) {
